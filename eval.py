...
for seq in seqs:
    ...
    for i, (im_rec, im_gt) in enumerate(zip(ims_rec, ims_gt)):
        with torch.no_grad():
            ...
            ### Resize the reconstructed frame to match the ground truth dimensions
            if gt.shape != rec.shape:
                rec = torch.nn.functional.interpolate(rec, size=gt.shape[2:], mode='bilinear', align_corners=False)
