...
controlnet_model = ControlNetModel.from_pretrained("/home/yuxin/StableVSR/experiments/model7_checkpoint-40000", subfolder='controlnet') # your own controlnet model
# controlnet_model = ControlNetModel.from_pretrained(args.controlnet_ckpt if args.controlnet_ckpt is not None else model_id, subfolder='controlnet')
pipeline = StableVSRPipeline.from_pretrained(model_id, controlnet=controlnet_model)
scheduler = DDPMScheduler.from_pretrained(model_id, subfolder='scheduler')

num_inference_steps = min(args.num_inference_steps, scheduler.config.num_train_timesteps - 1)
print(f"Final num_inference_steps: {num_inference_steps}")
...
frames = pipeline('', frames, num_inference_steps=num_inference_steps, guidance_scale=0, of_model=of_model).images
